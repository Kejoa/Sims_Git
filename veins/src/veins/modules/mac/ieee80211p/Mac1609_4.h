...
class Mac1609_4 : public BaseMacLayer,
	...
	public:
		...
		class EDCA {
			...
			public:
				...
				/** KM **/
                simtime_t tBSMsStartContention;
                simtime_t tBSMsStopContention;
                simtime_t tBSMsStartBO = 0;
                simtime_t tBSMPktWaiting;
                simtime_t tDENMsStartContention;
                simtime_t tDENMsStopContention;
                simtime_t tDENMsStartBO = 0;
                simtime_t tDENMPktWaiting;
                cOutVector ChannelAccessTime;
		};
    ...
    class NeighborhoodTable {
        public:
            ...
            int rcvdMsg_pwr;
            int senderNode_pos;
            std::string senderNode_dir;
            double senderNode_speed;
            simtime_t rcvdMsg_exp;

            std::vector<int> numKnownNodes;
            std::vector<int> SucRxdBSMsRefNodes;
            std::vector<int> numKnownRefNodes;
            std::vector<int> SucRxdBSMsMsgs;
            std::vector<int> SucRxdBSMsMsgsInInterval;
            std::vector<int> SucRxdDENMsMsg;
            std::queue<WaveShortMessage*> rcvdMsgs;
            NeighborhoodTable() {  };
            NeighborhoodTable(int rcvdMsg_id, int rcvdMsg_pwr, int senderNode_pos, std::string senderNode_dir, double senderNode_speed, simtime_t rcvdMsg_exp)
            : rcvdMsg_id(rcvdMsg_id), rcvdMsg_pwr(rcvdMsg_pwr), senderNode_pos(senderNode_pos), senderNode_dir(senderNode_dir), senderNode_speed(senderNode_speed), rcvdMsg_exp(rcvdMsg_exp) { };
    };
	...
	protected:
		...
		void oneHopNodeTable(cMessage* msg);
		void channelStateEstimation();
	...
	protected:
		...
		std::map<int,NeighborhoodTable> oneHopNodesTable;
		std::map<int,std::vector<int>> tempMsg_id;
		std::map<int,std::vector<int>> dispRxdMsgIds;
		std::map<int,std::vector<double>> CIV;

		bool idleChannel;

		/** @brief stats */
		long statsNumBSMsNeighbours;
		long statsNumDENMsNeighbours;
		long statsReceivedPackets;
		long statsReceivedPacketsInInterval;
		long statsReceivedBroadcasts;
		long statsReceivedBSMs;
		long statsReceivedBSMsInInterval;
		long statsNumberOfRefNodes;
		long statsReceivedBSMsfromRefNodes;
		long statsSentBSMs;
		long statsSentBSMsInInterval;
		long statsReceivedDENMs;
		long statsSentDENMs;
		long statsReSentDENMs;
		...
		simtime_t statsBSMsWaitingTime;
		simtime_t statsDENMsWaitingTime;
		double statsStartingPosition;
		...
		//
		simsignal_t sigChannelAccessDelay;
		simsignal_t sigChannelBusyRatio;
		simsignal_t sigBSMsReceived;
		simsignal_t sigCollidedPackets;
		simsignal_t sigOneHopNeighbours;

		TraCIMobility* MACmobility;
        TraCICommandInterface* MACtraci;
        TraCICommandInterface::Vehicle* MACtraciVehicle;
        Coord myStartPosition;
        double myPosition = 0;

        int sampleCount;
        int msgCount;
        int CurrentKnownVehicles;
        int currentCR;
        int PacketsReceived;
        int tempMsg[];
        simtime_t currentCBR;
        simtime_t BSMWaitingTime;
        double totCBR;
        bool pktSent;
