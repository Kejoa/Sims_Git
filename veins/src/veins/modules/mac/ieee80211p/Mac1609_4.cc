...
void Mac1609_4::initialize(int stage) {
	...
	if (stage == 1) {
	    ...
	    myID = findHost()->getIndex();
	    // To access mobility info of a vehicle
	    MACmobility = TraCIMobilityAccess().get(getParentModule()->getParentModule());
	    MACtraci = MACmobility->getCommandInterface();
	    MACtraciVehicle = MACmobility->getVehicleCommandInterface();
	    myStartPosition = MACmobility->getPositionAt(simTime());
	    myPosition = myStartPosition.x;
	    statsStartingPosition = myPosition;

	    sampleCount = 0;
	    externalLastBusy = simTime();
	    currentCBR = 0;
	    totCBR = 0;
	    PacketsReceived = 0;
	    pktSent = false;
	    everyInterval = par("everyInterval").boolValue();
	    everySec = par("everySec").boolValue();

	    sigChannelAccessDelay = registerSignal("sigChannelAccessDelay");
	    sigChannelBusyRatio = registerSignal("sigChannelBusyRatio");
	    sigBSMsReceived = registerSignal("sigBSMsReceived");
	    sigCollidedPackets = registerSignal("sigCollidedPackets");
	    sigOneHopNeighbours = registerSignal("sigOneHopNeighbours");

	    statsNumBSMsNeighbours = 0;
	    statsNumDENMsNeighbours = 0;
	    statsReceivedBSMs = 0;
	    statsReceivedBSMsInInterval = 0;
	    statsNumberOfRefNodes = 0;
	    statsReceivedBSMsfromRefNodes = 0;
	    statsSentBSMs = 0;
	    statsSentBSMsInInterval = 0;
	    statsReceivedDENMs = 0;
	    statsSentDENMs = 0;
	    statsReSentDENMs = 0;
	    statsBSMsWaitingTime = 0;
	    statsDENMsWaitingTime = 0;
	}
}
...
void Mac1609_4::handleUpperMsg(cMessage* msg) {
	...
	if(strcmp(thisMsg->getWsmData(),"reDENMs") == 0){
	        statsReSentDENMs++;
	}
...
void Mac1609_4::handleLowerMsg(cMessage* msg) {
	...
	if (macPkt->getDestAddr() == myMacAddress) {
		...
		if(strcmp("beacon", macPkt->getName()) == 0){
            statsReceivedBSMs++;
            oneHopNodeTable(wsm);
        }
        else{
...
/* Create a one-hop neighborhood table */
void Mac1609_4::oneHopNodeTable(cMessage* msg) {

    WaveShortMessage* tMsg = dynamic_cast<WaveShortMessage*>(msg);
    ASSERT(tMsg);

    int nodeId = tMsg->getSenderAddress();
    int rcvdMsg_id = tMsg->getSerial();
    int rcvdMsg_pwr = tMsg->getTx_power();
    int rcvdMsg_psid = tMsg->getPsid();

    Coord senderPos = tMsg->getSenderPos();
    int senderNode_pos = (int)(senderPos.x);
    std::string senderNode_dir = tMsg->getVehicleDirection(); //@KM necessary when dealing with retransmission of emergency messages
    // @KM - When considering mobility in the simulation
    double senderNode_speed = tMsg->getVehicleSpeed();
    simtime_t rcvdMsg_exp = simTime();

    NeighborhoodTable receivedData(rcvdMsg_id, rcvdMsg_pwr, senderNode_pos, senderNode_dir, senderNode_speed, rcvdMsg_exp);
    oneHopNodesTable[nodeId] = receivedData;
    if(rcvdMsg_psid == 0){
        oneHopNodesTable[myID].SucRxdBSMsMsgs.push_back(rcvdMsg_id);
        oneHopNodesTable[myID].SucRxdBSMsMsgsInInterval.push_back(nodeId);
        oneHopNodesTable[myID].numKnownNodes.push_back(nodeId);
        if((senderNode_pos >= 397) && (senderNode_pos <= 603)) {
            oneHopNodesTable[myID].SucRxdBSMsRefNodes.push_back(rcvdMsg_id);
            oneHopNodesTable[myID].numKnownRefNodes.push_back(nodeId);
        }
        oneHopNodesTable[nodeId].SucRxdBSMsMsgs.push_back(rcvdMsg_id);
    }
    else{
        oneHopNodesTable[myID].SucRxdDENMsMsg.push_back(nodeId);
    }
}

void Mac1609_4::channelStateEstimation() {

}

void Mac1609_4::debugDisplay() {
    DBG_MAC <<"-------------- Transmitter ------------------"<<std::endl;
    DBG_MAC <<"-------------- Parameter values ------------------"<<std::endl;
    DBG_MAC <<" "<<std::endl;
    int x = (pktSent == true)? 1: 0;
    DBG_MAC <<"--> Packet transmitted: "<<x<<std::endl;
    pktSent = false;
    DBG_MAC <<"--> Packet waiting time: " << BSMWaitingTime<<std::endl;
    DBG_MAC <<"--> Channel busy - time: " << currentCBR <<" - ratio: "<<currentCBR/0.50<<std::endl;
    currentCBR = 0;
    DBG_MAC <<" "<<std::endl;
    DBG_MAC <<"-------------- Receiver ------------------"<<std::endl;
    DBG_MAC <<"-------------- Parameter values ------------------"<<std::endl;
    DBG_MAC <<" "<<std::endl;
    DBG_MAC <<"--> Total # of known vehicles: " << CurrentKnownVehicles <<std::endl;
    DBG_MAC <<"--> Packet received: " << PacketsReceived <<std::endl;
    PacketsReceived = 0;
    DBG_MAC <<"--> Collision ratio: " << currentCR <<std::endl;
    currentCR = 0;
    DBG_MAC <<" "<<std::endl;
    if(sampleCount == 10){
        for(std::map<int,std::vector<int>>::iterator iter = dispRxdMsgIds.begin(); iter != dispRxdMsgIds.end(); iter++){
            if(iter->first != myID){
                DBG_MAC<<"----> Received Msgs from "<<iter->first<<" : ";
                for(unsigned int i=0; i < iter->second.size();i++){
                    DBG_MAC<<iter->second[i]<<" - ";
                }
                DBG_MAC<<" "<<std::endl;
            }
        }
...
void Mac1609_4::finish() {
	...
	// signal for stats collection for packet reception ratio
    for(std::map<int,NeighborhoodTable>::iterator iter = oneHopNodesTable.begin(); iter != oneHopNodesTable.end(); iter++){
        int Key = iter->first;
        if(Key == myID){
            std::vector<int> tempVec = iter->second.numKnownNodes;
            std::sort(tempVec.begin(),tempVec.end());
            tempVec.erase(std::unique(tempVec.begin(),tempVec.end()),tempVec.end());
            statsNumBSMsNeighbours = tempVec.size();

            tempVec = iter->second.SucRxdDENMsMsg;
            std::sort(tempVec.begin(),tempVec.end());
            tempVec.erase(std::unique(tempVec.begin(),tempVec.end()),tempVec.end());
            statsNumDENMsNeighbours = tempVec.size();

            tempVec = iter->second.SucRxdBSMsRefNodes;
            statsReceivedBSMsfromRefNodes = tempVec.size();

            tempVec = iter->second.numKnownRefNodes;
            std::sort(tempVec.begin(),tempVec.end());
            tempVec.erase(std::unique(tempVec.begin(),tempVec.end()),tempVec.end());
            statsNumberOfRefNodes = tempVec.size();
        }
    }
    oneHopNodesTable.clear();

    //stats
	recordScalar("StartingPosition",statsStartingPosition);
	//BSMs
    recordScalar("BSMsNeighbours",statsNumBSMsNeighbours);
    recordScalar("ReceivedBSMs",statsReceivedBSMs);
    recordScalar("SentBSMs",statsSentBSMs);
    recordScalar("totalBSMsWaitingTime",statsBSMsWaitingTime.dbl());
    //DENMs
    recordScalar("DENMsNeighbours",statsNumDENMsNeighbours);
    recordScalar("ReceivedDENMs",statsReceivedDENMs);
    recordScalar("SentDENMs",statsSentDENMs);
    recordScalar("ReSentDENMs",statsReSentDENMs);
    recordScalar("totalDENMsWaitingTime",statsDENMsWaitingTime.dbl());
    //Ref nodes
    recordScalar("NumberOfRefNodes",statsNumberOfRefNodes);
	recordScalar("BSMsFromRefNodes",statsReceivedBSMsfromRefNodes);
	//Network
